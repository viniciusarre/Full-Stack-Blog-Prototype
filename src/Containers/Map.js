...
const Map = () =>{
    ...
    if(data.length>0){
        return data.map((elem,ind)=>
            <Row key={ind}>
                <Col key={ind} md={12} style={{ boxShadow:'10px 10px 5px #888888', marginTop: 20, userSelect: 'none'}}>
                    ...
                    <Col md={6}>
                            <div style={{marginBottom: 10}}>
                                <Button bsStyle="warning" onClick={()=>UpdatePost(ind)}>
                                    <span className="glyphicon glyphicon-pencil"></span>
                                </Button>
                                <Button bsStyle="danger" style={{marginLeft: 10}} onClick={()=>Delete(ind)}>
